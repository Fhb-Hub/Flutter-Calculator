...
// Layout superior
  ...
  void _textOperation(String label) {
    final length = show.length;

    if (show == kZero && label != kPoint) show = kEmpty;
    if (show.contains(kPoint) && label == kPoint) label = kEmpty;

    if (kOperations.contains(label)) {
      if (show.endsWith('-') ||
          show.endsWith('+') ||
          show.endsWith('%') ||
          show.endsWith('/') ||
          show.endsWith('x')) {
        show = show.substring(0, length - 1);
        show += label;
      } else {
        show += label;
      }
